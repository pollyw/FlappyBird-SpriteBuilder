...
- (void)didLoadFromCCB {
    ...
    //move and look the bushes
    for(CCNode *bush in _bushes) {
        //move the bush
        bush.position = ccp(bush.position.x - (character.physicsBody.velocity.x * 0.5), bush.position.y);
        
        //if the left corner is on ecomplete width off the screen, move it to the right
    ...
    for (CCNode *cloud in _clouds) {
        ...
        cloud.position = ccp(cloud.position.x -
                             (character.physicsBody.velocity.x * 0.5), cloud.position.y);
